      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-10 items-center">
        {SlicedGames.map((game: any) => (
          <article key={game.id} onContextMenu={(e) => e.preventDefault()} className="text-center mt-7 w-full md:w-52 rounded-2xl p-2 bg-indigo-800 shadow-2xl">
            ...
            <Link to={`/games/${game.id}`} className={`${Menu ? 'hidden' : 'inline-flex'} text-center mt-3 cursor-pointer btn relative z-1 items-center justify-start overflow-hidden font-medium transition-all bg-transparent rounded hover:bg-white group py-2 px-3 border-2 border-purple-600 text-2xl text-white`}>
              ...
              <span className="relative w-full text-left text-white transition-colors duration-300 ease-in-out group-hover:text-white hover:border-0">Get Game</span>
            </Link>
          </article>
        ))}
      </div>
