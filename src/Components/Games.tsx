...
function Games({ Menu, SlicedGames }: { Menu: any, SlicedGames: any }) {
  ...
  return (
    <section className="select-none bg-indigo-700 p-5">
      ...
      <Link to="/games" className={`${Menu ? 'hidden' : 'bg-purple-400'} block shadow-2xl font-bold text-2xl cursor-pointer hover:bg-transparent hover:border-2 justify-self-start mt-8 border-2 border-purple-600 rounded text-white p-3 mt-3 w-full text-center`}>View All Games..</Link>
    </section>
  )
}
